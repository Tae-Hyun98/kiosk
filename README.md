...
#### 모달창은 처음에는 보이지않게 모달의 useState를 false로 상태를 정해놓고, 장바구니담기를 클릭시 모달의 state가 true로 변경되어 모달이 나타나며, 모달팝업에서 확인클릭시 장바구니에 상품을 추가하는 addCars가 호출되며, 닫기클릭시 state가 false로되어 모달창이 닫히도록 구현하였습니다. 
```javascript
